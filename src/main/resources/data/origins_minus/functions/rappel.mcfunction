execute at @e[type=snowball,tag=rappel] run playsound minecraft:entity.arrow.shoot player @p[distance=0..32] ~ ~ ~ 0.43 1.63
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~ ~
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~-0.2 ~
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~-0.4 ~
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~-0.6 ~
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~-0.8 ~
execute at @e[type=snowball,tag=rappel] run particle minecraft:block minecraft:cobweb ~ ~1 ~
execute as @e[type=snowball,tag=rappel,nbt={PortalCooldown:0}] run kill @s