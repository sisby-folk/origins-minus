...
execute at @e[type=arrow,tag=bioluminescence,nbt={inGround:1b}] run particle block{block_state:{Name:'minecraft:glow_lichen'}} ~ ~ ~ 0.5 0.5 0.5 0 32
execute as @e[type=arrow,tag=bioluminescence,nbt={inGround:1b}] run kill @s

execute at @e[type=arrow,tag=bioluminescence] run particle block{block_state:{Name:'minecraft:glow_lichen'}} ~ ~ ~ 0.1 0.1 0.1 0 8
